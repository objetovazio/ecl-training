...
AlphaPay := INDEX(Ptbl2,{lname,fname},{Ptbl2},KeyFile2); //paylaod
...
joinedRecord doJoin(peopleRecord Le, Ptbl1 Ri) := TRANSFORM
  SELF := Le;
  SELF := Ri;
END;
...
Out1 := OUTPUT(FilledRecs1,NAMED('FilledRecs1'));
Out2 := OUTPUT(FilledRecs2,NAMED('FilledRecs2'));
Out3 := OUTPUT(FilledRecs3,NAMED('FilledRecs3'));
Out4 := OUTPUT(FilledRecs4,NAMED('FilledRecs4'));

SEQUENTIAL(PtblOut,PARALLEL(Bld1,Bld2,Bld3,Bld4),PARALLEL(Out1,Out2,Out3,Out4));
